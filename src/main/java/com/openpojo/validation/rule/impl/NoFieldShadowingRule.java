...
public class NoFieldShadowingRule implements Rule {
    ...
    private boolean inSkipList(PojoField field) {
        for (String skipEntry : fieldNamesToSkip) if (skipEntry.equals(field.getName()))
            return true;
        return false;
    }

    private boolean isSerializable(PojoField field, PojoClass pojoClass) {
        return field.getName().equals(SERIAL_VERSION_UID_FIELD_NAME) && Serializable.class.isAssignableFrom(pojoClass.getClazz()) && field.getType().equals(SERIAL_VERSION_UID_FIELD_TYPE);
    }
