...
public class InstanceFactory {
    ...
    private static Object doGetInstance(PojoClass pojoClass, Object[] parameters) {
        ...
        throw ReflectionException.getInstance(String.format("No matching constructor for [%s] found using parameters[%s]", pojoClass.getClazz(), Arrays.toString(getTypes(parameters))));
    }
    ...
    private static Class<?>[] upCast(final Class<?>[] parameterTypes) {
        ...
        for (int idx = 0; idx < parameterTypes.length; idx++) upCastedParameters[idx] = Primitives.getInstance().autoBox(parameterTypes[idx]);
    ...
    private static Object[] generateRandomValuesForParameters(final PojoClass pojoClass, final PojoMethod constructor) {
        try {
            ...
            for (int i = 0; i < pojoParameterTypes.size(); i++) parameters[i] = RandomFactory.getRandomValue(pojoParameterTypes.get(i));
            return parameters;
        } catch (RuntimeException re) {
            throw ReflectionException.getInstance("Failed to create parameters to pass to constructor for class [" + pojoClass.getName() + "] using constructor [" + constructor + "]", re);
        }
    }
